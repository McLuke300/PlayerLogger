...
public class PlayerLoggerListener implements Listener{
		...
		if(event.getResult() == PlayerLoginEvent.Result.ALLOWED) {
			...
			if (plugin.getConfig().getBoolean("Log.PlayerJoins")) {
				...
				if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
					playerlogger.addData(playername,"join", ip, x, y, z, worldname, staff);
				}}}}
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		if (plugin.getConfig().getBoolean("Log.PlayerQuit")) {
			...
			if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
				playerlogger.addData(playername,"quit", "", x, y, z, worldname, staff);	
			}}}
		...
		if (plugin.getConfig().getBoolean("Log.PlayerCommands")) {
			if (plugin.getConfig().getBoolean("Commands.BlackListCommands")) {
				...
				if (!log) {
					...
					if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
						playerlogger.addData(playername,"command", msg, x, y, z, worldname, staff);
					}}
			}else {
				if (player.hasPermission("PlayerLogger.staff")) {
					staff = true;
	...
	public void onEntityDeath(EntityDeathEvent event){
		...
		if(ent instanceof Player){
			...
			if (plugin.getConfig().getBoolean("Log.PlayerDeaths")) {
				...
				if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
					playerlogger.addData(playername,"death", "", x, y, z, worldname, staff);
				}}}}
	...
	public void onEnchant(EnchantItemEvent event){
		...
		if (plugin.getConfig().getBoolean("Log.PlayerEnchants")) {
			...
			if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
				playerlogger.addData(playername,"enchant", item+" "+ench+" Xp Cost:"+cost, 0, 0, 0, worldname, staff);
			}}}
		...
		if (plugin.getConfig().getBoolean("Log.PlayerSignText")) {
			if (event.isCancelled() == false) {
				if (event.getBucket() != null && event.getBucket()==Material.LAVA_BUCKET) {
					...
					if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
						playerlogger.addData(playername,"bucket", "Lava", x, y, z, worldname, staff);
					}
					...
					if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
						playerlogger.addData(playername,"bucket", "Water", x, y, z, worldname, staff);
					}}}}}
	...
	public void onSign(SignChangeEvent event){
		...
		if (event.isCancelled() == false) {
			if (plugin.getConfig().getBoolean("Log.PlayerSignText")) {
				...
				if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
					playerlogger.addData(playername,"sign", "["+lines[0]+"]"+"["+lines[1]+"]"+"["+lines[2]+"]"+"["+lines[3]+"]", x, y, z, worldname, staff);
				}}}}
			...
			if(ply instanceof Player){            
				if(dmgr instanceof Player){
					...
					if (plugin.getConfig().getBoolean("Log.Pvp")) {
						...
						if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
							playerlogger.addData(damager,"kill", "Killed "+player, x, y, z, worldname, staff);
						}}}}}}
	...
	public void onPlayerConsoleCommand(ServerCommandEvent event) {
		...
		if (plugin.getConfig().getBoolean("Log.ConsoleCommands") && plugin.getConfig().getBoolean("MySQL.Enabled")) {
			playerlogger.addData("","console", msg, 0, 0, 0, "", true);
		}}
		...
		if (event.isCancelled() == false) {
			if(plugin.getConfig().getBoolean("BlackList.LogBlackListedBlocks")) {
				...
				if (log) {
					...
					if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
						playerlogger.addData(playername,"place", blockname, x, y, z, worldname, staff);
					}}}}}
		...
		if (event.isCancelled() == false) {
			if(plugin.getConfig().getBoolean("BlackList.LogBlackListedBlocks")) {
				...
				if (log) {
					...
					if (plugin.getConfig().getBoolean("MySQL.Enabled")) {
						playerlogger.addData(playername,"break", blockname, x, y, z, worldname, staff);
					}}}}}
